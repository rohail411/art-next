...
const AudioUpload = ({
    ...
    function _createRequest( file, isVideo) {
        return new Promise((resolve, reject) => {
            ...
            uploadRequest.addEventListener(
                ...
                function () {
                    if (uploadRequest.readyState === 4) {
                        ...
                        if (result.IpfsHash) {
                            if (isVideo) {
                                setProgress({ value: 100, show: true, message: 'Audio Complete!' });
                            } else {
                                setProgress({
                                    value: 100,
                                    show: true,
            ...
            uploadRequest.setRequestHeader("pinata_api_key", process.env.NEXT_PUBLIC_PINATA_PUBLIC_KEY);
            uploadRequest.setRequestHeader("pinata_secret_api_key", process.env.NEXT_PUBLIC_PINATA_PRIVATE_KEY);
    ...
    const uploadAudioFiles = async () => {
        ...
        for (let i = 0; i < audioTracks.length; i++) {
            ...
            file.uploading = true;
            files[i] = file;
            const media = await _createRequest(file, true);
    ...
    const submitHandler = async (e) => {
        ...
        const media = await _createRequest(imgComp,false);
        if (media.path === null) {
